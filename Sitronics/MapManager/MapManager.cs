...
namespace Sitronics.MapManager
{
    public static class MapManager
    {
        ...
        public static GMapMarker CreateBusMarker(PointLatLng point, ref GMapControl mapView, Bus bus, GMapMarker? lastMarker = null)
        {
            ...
            var peopleInBus = bus.RouteHasBus.FirstOrDefault(rhb=>rhb.IdBusNavigation == bus)
                        .Schedules.Sum(s => s.PeopleCountBoardingBus) -
                        bus.RouteHasBus.FirstOrDefault(rhb => rhb.IdBusNavigation == bus)
                        .Schedules.Sum(s => s.PeopleCountGettingOffBus);
